...
const STANDARD_YEARS: Record<string, string> = { '9001': '2015', '14001': '2015', '45001': '2018' }
const formatClauseLabel = (c: IsoClause) => `ISO ${c.standard}:${STANDARD_YEARS[c.standard]} Clause ${c.number} – ${c.title}`
...
const Sidebar = ({ role, userEmail, onLogout }: { role: UserRole; userEmail: string; onLogout?: () => void }) => {
  ...
  const navItem = (href: string, icon: React.ReactNode, label: string, key: string, badge?: number | string, tierCode?: string, active = false) => (
    <Link key={key} href={href} className={`flex items-center gap-3 px-3 py-2.5 rounded-lg mb-0.5 transition-all ${active ? 'bg-emerald-50 text-emerald-950 shadow-sm' : 'text-emerald-100 hover:bg-emerald-800/50'}`}>
      ...
      {badge !== undefined && <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-full min-w-[20px] text-center ${active ? 'bg-emerald-600 text-white' : 'bg-emerald-800 text-emerald-400'}`}>{badge}</span>}
    </Link>
  )
  return (
...
const Topbar = ({
  crumb, userEmail, role, isAdmin,
  showSharePanel, setShowSharePanel,
  shareEmail, setShareEmail,
  sharePermission, setSharePermission,
  shareLinkPermission, setShareLinkPermission,
  shareLink, shareMessage,
  handleShareByEmail, handleGenerateLink, shareLoading,
  searchQuery, setSearchQuery,
}: {
  crumb: string; userEmail: string; role: UserRole; isAdmin: boolean
  showSharePanel: boolean; setShowSharePanel: (v: boolean) => void
  shareEmail: string; setShareEmail: (v: string) => void
  sharePermission: 'view' | 'edit'; setSharePermission: (v: 'view' | 'edit') => void
  shareLinkPermission: 'view' | 'edit'; setShareLinkPermission: (v: 'view' | 'edit') => void
  shareLink: string; shareMessage: string
  handleShareByEmail: () => void; handleGenerateLink: () => void
  shareLoading: boolean; searchQuery: string; setSearchQuery: (v: string) => void
}) => {
  const initial = userEmail ? userEmail[0].toUpperCase() : 'N'
  const roleLabel = role === 'admin' ? 'Admin' : role === 'editor' ? 'Editor' : 'Viewer'
  return (
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="pl-9 pr-8 py-2 text-sm bg-emerald-50 border border-emerald-200 rounded-lg text-emerald-950 placeholder:text-emerald-400 focus:outline-none focus:ring-2 focus:ring-emerald-500/30 focus:border-emerald-500 transition w-48"
          />
          {searchQuery && (
            <button onClick={() => setSearchQuery('')} className="absolute right-2 top-1/2 -translate-y-1/2 text-emerald-400 hover:text-emerald-700">
        ...
        {isAdmin && (
          <div className="relative">
            <button onClick={() => setShowSharePanel(!showSharePanel)} className="flex items-center gap-2 px-3 py-2 bg-emerald-700 hover:bg-emerald-800 text-white text-sm font-medium rounded-lg transition shadow-sm">
  ...
  {shareLoading ? '...' : 'Invite'}
                ...
                {shareLink && (
                  <div className="flex gap-2">
                    ...
                    <button onClick={() => navigator.clipboard.writeText(shareLink)} className="px-3 py-2 border border-emerald-200 rounded-lg text-sm text-emerald-700 hover:bg-emerald-50">Copy</button>
                  </div>
                )}
                {shareMessage && (
                  <p className={`text-xs mt-2 ${shareMessage.includes('granted') || shareMessage.includes('Copied') ? 'text-emerald-600' : 'text-rose-600'}`}>{shareMessage}</p>
                )}
              </div>
            )}
          </div>
...
const IsoElementCombobox = ({ value, onChange, disabled }: { value: string; onChange: (v: string) => void; disabled?: boolean }) => {
  ...
  const filtered = useMemo(() => {
    ...
    return ISO_CLAUSES.filter((c) => { const label = formatClauseLabel(c).toLowerCase(); return label.includes(q) || c.number.toLowerCase().includes(q) || c.standard.includes(q) })
  }, [query])
  ...
  const standardLabels: Record<string, string> = { '9001': 'ISO 9001:2015 — Quality Management', '14001': 'ISO 14001:2015 — Environmental Management', '45001': 'ISO 45001:2018 — Occupational Health & Safety' }
  const selectClause = (c: IsoClause) => { const label = formatClauseLabel(c); setQuery(label); onChange(label); setOpen(false) }
  const indent = (c: IsoClause) => (c.number.match(/\./g) || []).length
  return (
    <div ref={containerRef} className="relative">
      <div className="flex items-center gap-2">
        <input type="text" value={query} onChange={(e) => { setQuery(e.target.value); onChange(e.target.value); setOpen(true) }} onFocus={() => setOpen(true)} disabled={disabled} placeholder="Type to search clauses…" className="w-full px-3 py-2.5 pr-10 rounded-lg border border-emerald-200 bg-white text-sm text-emerald-900 placeholder-emerald-300 focus:outline-none focus:ring-2 focus:ring-emerald-400/30 focus:border-emerald-400 transition-all disabled:bg-emerald-50/60 disabled:cursor-not-allowed" />
        <button type="button" onClick={() => !disabled && setOpen(!open)} disabled={disabled} className="absolute right-2 top-1/2 -translate-y-1/2 p-1 text-emerald-500 hover:text-emerald-700 transition disabled:opacity-50"><ChevronDownIcon className={`w-4 h-4 transition-transform ${open ? 'rotate-180' : ''}`} /></button>
      </div>
      {open && (
        <div className="absolute z-20 mt-1 w-full bg-white border border-emerald-200 rounded-lg shadow-lg max-h-80 overflow-y-auto">
          {filtered.length === 0 ? (
            <div className="px-4 py-6 text-center text-sm text-emerald-500">No clauses match "{query}".</div>
          ) : (
            (['9001', '14001', '45001'] as const).map((std) => {
              ...
              return (
                <div key={std} className="border-b border-emerald-50 last:border-0">
                  <div className="px-3 py-2 text-[10px] font-semibold uppercase tracking-wider text-emerald-700 bg-emerald-50/80 sticky top-0">{standardLabels[std]}</div>
                  {items.map((c) => (
                    <button key={`${std}-${c.number}`} type="button" onClick={() => selectClause(c)} className="w-full text-left px-3 py-2 hover:bg-emerald-50 transition flex items-baseline gap-2" style={{ paddingLeft: `${12 + indent(c) * 14}px` }}>
                      <span className="font-mono text-xs font-semibold text-emerald-700 shrink-0 min-w-[3.5rem]">{c.number}</span>
...
const SectionHeader = ({ children }: { children: React.ReactNode }) => (
  <div className="bg-emerald-100/60 border-y border-emerald-200 px-5 py-2 text-xs font-semibold text-emerald-900 uppercase tracking-wider">{children}</div>
)
...
const ReadField = ({ label, value }: { label: string; value?: string | null }) => (
  <div>
    <label className="block text-[11px] font-semibold tracking-widest uppercase text-emerald-500 mb-1.5">{label}</label>
    <div className="bg-emerald-50/60 border border-emerald-200 rounded-lg px-3 py-2.5 min-h-[42px] text-sm text-emerald-900 whitespace-pre-wrap leading-relaxed">{value || '—'}</div>
  </div>
)
const SeverityToggle = ({ value, onChange, disabled }: { value: Severity; onChange: (v: Severity) => void; disabled?: boolean }) => (
  <div className="flex gap-3">
    {(['Major', 'Minor'] as const).map((opt) => {
      const active = value === opt
      return (
        <button key={opt} type="button" disabled={disabled} onClick={() => onChange(active ? null : opt)} className={`flex items-center gap-2 px-4 py-2 rounded-lg border-2 transition-all text-sm font-medium ${active ? opt === 'Major' ? 'bg-rose-50 border-rose-400 text-rose-800' : 'bg-amber-50 border-amber-400 text-amber-800' : 'bg-white border-emerald-200 text-emerald-700 hover:border-emerald-400'} disabled:opacity-50 disabled:cursor-not-allowed`}>
          <span className={`w-4 h-4 rounded border-2 flex items-center justify-center shrink-0 ${active ? opt === 'Major' ? 'bg-rose-500 border-rose-500' : 'bg-amber-500 border-amber-500' : 'border-emerald-300 bg-white'}`}>{active && <CheckIcon className="w-3 h-3 text-white" />}</span>
          {opt}
        </button>
      )
    })}
...
const SignatureUpload = ({ label, file, onFileChange, signedUrl, disabled }: { label: string; file: File | null; onFileChange: (f: File | null) => void; signedUrl?: string | null; disabled?: boolean }) => {
  ...
  return (
    <div>
      ...
      <label htmlFor={inputId} className={`block border-2 border-dashed rounded-lg p-4 text-center transition ${disabled ? 'border-emerald-100 bg-emerald-50/30 cursor-not-allowed' : 'border-emerald-200 hover:border-emerald-400 hover:bg-emerald-50/40 cursor-pointer'}`}>
        <input id={inputId} type="file" accept="image/png,image/jpeg,.png,.jpg,.jpeg" className="hidden" disabled={disabled} onChange={(e) => onFileChange(e.target.files?.[0] || null)} />
        {previewSrc ? (
        ...
        ) : <div className="text-sm text-emerald-500 py-2">Click to upload signature image (PNG / JPG)</div>}
      </label>
    </div>
  )
}
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      particles.forEach((p) => {
        ...
        ctx.beginPath(); ctx.arc(p.x, p.y, p.radius, 0, Math.PI * 2); ctx.fillStyle = 'rgba(167, 243, 208, 0.6)'; ctx.fill()
      })
      ...
      for (let i = 0; i < particles.length; i++) {
        for (let j = i + 1; j < particles.length; j++) {
          ...
          if (distance < maxDistance) { const opacity = (1 - distance / maxDistance) * 0.3; ctx.beginPath(); ctx.moveTo(particles[i].x, particles[i].y); ctx.lineTo(particles[j].x, particles[j].y); ctx.strokeStyle = `rgba(110, 231, 183, ${opacity})`; ctx.lineWidth = 0.6; ctx.stroke() }
        }
      }
    ...
    <main className="min-h-screen flex items-center justify-center px-4 relative overflow-hidden" style={{ fontFamily: "'Inter', system-ui, sans-serif", background: 'radial-gradient(ellipse at top right, #0F5A35 0%, #094A2A 50%, #063B22 100%)' }}>
      ...
      <div className="relative flex flex-col items-center gap-8" style={{ zIndex: 2 }}>
        ...
        <div className="w-64 h-1 bg-white/10 rounded-full overflow-hidden">
          <div className="h-full bg-gradient-to-r from-emerald-400 via-emerald-200 to-emerald-400 rounded-full" style={{ backgroundSize: '200% 100%', animation: 'ncr-shimmer 2s linear infinite' }} />
        </div>
      ...
      <style>{`@keyframes ncr-shimmer { 0% { background-position: 200% 0; } 100% { background-position: -200% 0; } }`}</style>
    </main>
  )
}
...
export default function NCRFormPage() {
  ...
  const [form, setForm] = useState({
    title: '', auditDate: todayDate, isoElement: '', severity: null as Severity,
    description: '', internalAuditors: '', acknowledgedBy: '', rootCause: '',
    correctiveAction: '', agreedDueDate: '', responsibility: '', verificationText: '',
    auditorName: '', dateClosed: '', dateRaised: now, targetCloseDate: '',
    preparedBy: null as string | null, preparedSignedAt: null as string | null,
    approvedBy: null as string | null, approvedSignedAt: null as string | null,
  })
  ...
  const init = async () => {
    ...
    if (profileRole === 'admin') { setRole('admin'); setLoading(false); return }
    if (profileRole === 'co-admin') { setRole('editor'); setLoading(false); return }

    const { data: editorData } = await supabase.from('ncr_editors').select('email').ilike('email', email).maybeSingle()
    if (editorData) { setRole('editor'); setLoading(false); return }
  ...
  const loadNCRsForViewer = async () => {
    const { data, error } = await supabase.from('ncrs').select('*').order('date_raised', { ascending: false })
  ...
  const uploadSignature = async (file: File, ncrNumber: string, kind: 'auditor' | 'ack'): Promise<string | null> => {
    ...
    if (error) { console.error('Signature upload error:', error.message); return null }
    return path
  }
  ...
  const submitNCR = async () => {
    ...
    try {
      ...
      const { error } = await supabase.from('ncrs').insert({
        ...
        description: form.description, root_cause: form.rootCause, corrective_action: form.correctiveAction,
        date_raised: form.dateRaised, target_close_date: form.targetCloseDate, created_by: userEmail,
        prepared_by: form.preparedBy, prepared_signed_at: form.preparedSignedAt,
        approved_by: form.approvedBy, approved_signed_at: form.approvedSignedAt,
        audit_date: form.auditDate || null, iso_element: form.isoElement || null, severity: form.severity,
        internal_auditors: form.internalAuditors || null, acknowledged_by: form.acknowledgedBy || null,
        responsibility: form.responsibility || null, agreed_due_date: form.agreedDueDate || null,
        verification_text: form.verificationText || null, auditor_name: form.auditorName || null,
        date_closed: form.dateClosed || null, auditor_signature_path: auditorPath, acknowledger_signature_path: ackPath,
      })
      if (error) { alert('Error saving NCR: ' + error.message) } else {
        alert('NCR saved successfully')
        setForm({ ...form, title: '', isoElement: '', severity: null, description: '', internalAuditors: '', acknowledgedBy: '', rootCause: '', correctiveAction: '', agreedDueDate: '', responsibility: '', verificationText: '', auditorName: '', dateClosed: '', targetCloseDate: '', preparedBy: null, preparedSignedAt: null, approvedBy: null, approvedSignedAt: null })
        setAuditorSig(null); setAckSig(null)
      }
...
const handleShareByEmail = async () => {
  ...
  try {
    // First try to invite — creates account + sends email + grants access
    ...
    if (res.ok) {
      ...
      if (!profile) {
        setShareMessage('User not found.')
  ...
  const handleGenerateLink = async () => {
    ...
    const { error } = await supabase.from('page_access').insert({ page: '/ncr', user_id: null, permission: shareLinkPermission, share_token: token })
  ...
  const filteredNcrList = searchQuery.trim()
    ? ncrList.filter(n =>
        n.title.toLowerCase().includes(searchQuery.toLowerCase()) ||
        n.ncr_number.toLowerCase().includes(searchQuery.toLowerCase()) ||
        (n.iso_element || '').toLowerCase().includes(searchQuery.toLowerCase())
      )
    : ncrList

  const topbarProps = {
    crumb: 'Non-Conformance', userEmail, role, isAdmin,
    showSharePanel, setShowSharePanel,
    shareEmail, setShareEmail,
    sharePermission, setSharePermission,
    shareLinkPermission, setShareLinkPermission,
  ...
  const shell = (children: React.ReactNode) => (
    <div className="min-h-screen bg-emerald-50/40 text-emerald-950 flex" style={{ fontFamily: "'Inter', system-ui, sans-serif" }}>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <Topbar {...topbarProps} />
        <div className="flex-1 p-8 flex flex-col gap-5 max-w-5xl">{children}</div>
      </div>
    </div>
  )
  ...
  if (isViewer) {
    return shell(
      selectedNCR ? (
        <>
          <div>
            <button type="button" onClick={() => setSelectedNCR(null)} className="px-3 py-1.5 text-xs font-medium border border-emerald-200 text-emerald-600 rounded-lg hover:bg-emerald-50 transition-all">← Back to list</button>
          </div>
          <div>
          ...
          <div className="bg-white border border-emerald-200 rounded-xl shadow-sm overflow-hidden">
            <div className="grid grid-cols-2 gap-0 border-b border-emerald-200">
              <div className="p-5 border-r border-emerald-200 space-y-4"><ReadField label="Audit Date" value={selectedNCR.audit_date} /></div>
              <div className="p-5 space-y-4"><ReadField label="ISO Element" value={selectedNCR.iso_element} /></div>
            </div>
                ...
                <div className="text-sm text-emerald-900">
                  {selectedNCR.severity ? (
                    <span className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-medium ${selectedNCR.severity === 'Major' ? 'bg-rose-50 text-rose-800 border border-rose-200' : 'bg-amber-50 text-amber-800 border border-amber-200'}`}>
                      <CheckIcon className="w-3 h-3" />{selectedNCR.severity}
                    </span>
            ...
            <div className="grid grid-cols-2 gap-0 border-b border-emerald-200">
              <div className="p-5 border-r border-emerald-200 space-y-3">
                <div>
                  ...
                  <div className="bg-emerald-50/60 border border-emerald-200 rounded-lg p-3 min-h-[100px] flex items-center justify-center">
                    {signaturePreviews.auditor ? <img src={signaturePreviews.auditor} alt="Auditor signature" className="max-h-20 object-contain" /> : <span className="text-xs text-emerald-400 italic">No signature on file</span>}
                  </div>
                </div>
              ...
              <div className="p-5 space-y-3">
                <div>
                  ...
                  <div className="bg-emerald-50/60 border border-emerald-200 rounded-lg p-3 min-h-[100px] flex items-center justify-center">
                    {signaturePreviews.ack ? <img src={signaturePreviews.ack} alt="Acknowledger signature" className="max-h-20 object-contain" /> : <span className="text-xs text-emerald-400 italic">No signature on file</span>}
                  </div>
                </div>
        ...
        <>
          ...
          <div className="flex items-start gap-3 p-4 bg-amber-50 border border-amber-200 rounded-lg">
            ...
            <p className="text-sm text-amber-800"><strong>Viewer Mode — Read Only.</strong> Contact your administrator to request edit or submission permissions.</p>
          </div>
          <div className="bg-white border border-emerald-100 rounded-xl shadow-sm overflow-hidden">
            <div className="flex items-center justify-between px-5 py-3.5 border-b border-emerald-100">
              ...
              <span className="text-xs text-emerald-400">{filteredNcrList.length} report{filteredNcrList.length !== 1 ? 's' : ''}</span>
            </div>
            {filteredNcrList.length === 0 ? (
              ...
              <table className="w-full border-collapse">
                <thead>
                  <tr>{['NCR Number', 'Title', 'Severity', 'ISO Element', 'Date Raised'].map(h => (
                  ...
                  {filteredNcrList.map((ncr) => (
                    <tr key={ncr.id} className="cursor-pointer hover:bg-emerald-50/60 transition-all" onClick={() => setSelectedNCR(ncr)}>
                      ...
                      <td className="px-4 py-3 border-b border-emerald-50 text-sm">
                        {ncr.severity ? <span className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[11px] font-medium ${ncr.severity === 'Major' ? 'bg-rose-100 text-rose-800' : 'bg-amber-100 text-amber-800'}`}>{ncr.severity}</span> : <span className="text-emerald-400">—</span>}
                      </td>
  ...
  return shell(
    <>
      <div>
        ...
        <p className="text-sm text-emerald-500 mt-1">Logged in as <strong className="font-semibold text-emerald-700">{userEmail}</strong> · {role}</p>
      </div>
      ...
      <div className="bg-white border border-emerald-200 rounded-xl shadow-sm overflow-hidden">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-0 border-b border-emerald-200">
          <div className="p-5 md:border-r border-emerald-200">
            <Field label="Audit Date">
              <input type="date" name="auditDate" value={form.auditDate} onChange={handleChange} className="w-full px-3 py-2.5 rounded-lg border border-emerald-200 bg-white text-sm text-emerald-900 focus:outline-none focus:ring-2 focus:ring-emerald-400/30 focus:border-emerald-400 transition-all" />
            </Field>
          </div>
          <div className="p-5">
        ...
        <SectionHeader>Description of the Non-Conformance</SectionHeader>
        <div className="p-5 space-y-4">
          <Field label="Severity"><SeverityToggle value={form.severity} onChange={(v) => setForm({ ...form, severity: v })} /></Field>
          <Field label="Title">
            <input name="title" value={form.title} onChange={handleChange} placeholder="Brief title for this finding" className="w-full px-3 py-2.5 rounded-lg border border-emerald-200 bg-white text-sm text-emerald-900 placeholder-emerald-300 focus:outline-none focus:ring-2 focus:ring-emerald-400/30 focus:border-emerald-400 transition-all" />
          </Field>
          <Field label="Description">
            <textarea name="description" value={form.description} onChange={handleChange} placeholder="Describe the non-conformance in detail…" rows={4} className="w-full px-3 py-2.5 rounded-lg border border-emerald-200 bg-white text-sm text-emerald-900 placeholder-emerald-300 focus:outline-none focus:ring-2 focus:ring-emerald-400/30 focus:border-emerald-400 transition-all resize-y" />
          </Field>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-0 border-b border-emerald-200">
          <div className="p-5 md:border-r border-emerald-200 space-y-4">
            ...
            <Field label="Internal Auditors">
              <input name="internalAuditors" value={form.internalAuditors} onChange={handleChange} placeholder="e.g. JO / HM" className="w-full px-3 py-2.5 rounded-lg border border-emerald-200 bg-white text-sm text-emerald-900 placeholder-emerald-300 focus:outline-none focus:ring-2 focus:ring-emerald-400/30 focus:border-emerald-400 transition-all" />
            </Field>
          </div>
          <div className="p-5 space-y-4">
            <SignatureUpload label="Acknowledgment Signature" file={ackSig} onFileChange={setAckSig} />
            <Field label="Acknowledged By">
              <input name="acknowledgedBy" value={form.acknowledgedBy} onChange={handleChange} placeholder="e.g. Aamir Zahoor" className="w-full px-3 py-2.5 rounded-lg border border-emerald-200 bg-white text-sm text-emerald-900 placeholder-emerald-300 focus:outline-none focus:ring-2 focus:ring-emerald-400/30 focus:border-emerald-400 transition-all" />
            </Field>
          </div>
        </div>
        ...
        <div className="p-5">
          <Field label="Root Cause">
            <textarea name="rootCause" value={form.rootCause} onChange={handleChange} placeholder="Identify the root cause…" rows={3} className="w-full px-3 py-2.5 rounded-lg border border-emerald-200 bg-white text-sm text-emerald-900 placeholder-emerald-300 focus:outline-none focus:ring-2 focus:ring-emerald-400/30 focus:border-emerald-400 transition-all resize-y" />
          </Field>
        </div>
        ...
        <div className="p-5 space-y-4">
          <Field label="Corrective Action">
            <textarea name="correctiveAction" value={form.correctiveAction} onChange={handleChange} placeholder="Describe corrective actions to be taken…" rows={3} className="w-full px-3 py-2.5 rounded-lg border border-emerald-200 bg-white text-sm text-emerald-900 placeholder-emerald-300 focus:outline-none focus:ring-2 focus:ring-emerald-400/30 focus:border-emerald-400 transition-all resize-y" />
          </Field>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Field label="Agreed Due Date">
              <input name="agreedDueDate" value={form.agreedDueDate} onChange={handleChange} placeholder="e.g. Q2 2026 or 30 Jun 2026" className="w-full px-3 py-2.5 rounded-lg border border-emerald-200 bg-white text-sm text-emerald-900 placeholder-emerald-300 focus:outline-none focus:ring-2 focus:ring-emerald-400/30 focus:border-emerald-400 transition-all" />
            </Field>
            <Field label="Responsibility">
              <input name="responsibility" value={form.responsibility} onChange={handleChange} placeholder="e.g. Contracts / Procurement Department" className="w-full px-3 py-2.5 rounded-lg border border-emerald-200 bg-white text-sm text-emerald-900 placeholder-emerald-300 focus:outline-none focus:ring-2 focus:ring-emerald-400/30 focus:border-emerald-400 transition-all" />
            </Field>
          </div>
        </div>
        ...
        <div className="p-5">
          <Field label="(Please attach evidence, results of action taken, actions completed)">
            <textarea name="verificationText" value={form.verificationText} onChange={handleChange} placeholder="Describe verification of corrective action effectiveness…" rows={3} className="w-full px-3 py-2.5 rounded-lg border border-emerald-200 bg-white text-sm text-emerald-900 placeholder-emerald-300 focus:outline-none focus:ring-2 focus:ring-emerald-400/30 focus:border-emerald-400 transition-all resize-y" />
          </Field>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-5">
          <Field label="Auditor Name & Signature">
            <input name="auditorName" value={form.auditorName} onChange={handleChange} placeholder="e.g. Jordain Opone / Harlene Manansala" className="w-full px-3 py-2.5 rounded-lg border border-emerald-200 bg-white text-sm text-emerald-900 placeholder-emerald-300 focus:outline-none focus:ring-2 focus:ring-emerald-400/30 focus:border-emerald-400 transition-all" />
          </Field>
          <Field label="Date Closed">
            <input name="dateClosed" value={form.dateClosed} onChange={handleChange} placeholder="e.g. In Progress, or 15 Mar 2026" className="w-full px-3 py-2.5 rounded-lg border border-emerald-200 bg-white text-sm text-emerald-900 placeholder-emerald-300 focus:outline-none focus:ring-2 focus:ring-emerald-400/30 focus:border-emerald-400 transition-all" />
          </Field>
        </div>
      </div>
      ...
      <div className="bg-white border border-emerald-100 rounded-xl shadow-sm overflow-hidden">
        ...
        <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="p-4 bg-emerald-50/60 border border-emerald-200 rounded-lg">
            ...
            {form.preparedBy ? (
              <div className="text-sm text-emerald-700">
                <span className="inline-flex items-center gap-1.5 text-emerald-600 font-medium"><CheckIcon className="w-3.5 h-3.5" /> Signed by <strong>{form.preparedBy}</strong></span><br />
              ...
              <button type="button" onClick={signAsPrepared} className="mt-1 px-4 py-1.5 border border-emerald-300 text-emerald-700 text-xs font-medium rounded-lg hover:bg-emerald-100 transition-all">Sign as Prepared By</button>
          ...
          <div className="p-4 bg-emerald-50/60 border border-emerald-200 rounded-lg">
            ...
            {form.approvedBy ? (
              <div className="text-sm text-emerald-700">
                <span className="inline-flex items-center gap-1.5 text-emerald-600 font-medium"><CheckIcon className="w-3.5 h-3.5" /> Signed by <strong>{form.approvedBy}</strong></span><br />
              ...
              <button type="button" onClick={signAsApproved} className="mt-1 px-4 py-1.5 border border-emerald-300 text-emerald-700 text-xs font-medium rounded-lg hover:bg-emerald-100 transition-all">Sign & Approve NCR</button>
      ...
      <div>
        <button type="button" onClick={submitNCR} disabled={submitting} className="px-6 py-2.5 bg-emerald-700 hover:bg-emerald-800 text-white text-sm font-semibold rounded-full transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2">
