...
// Tier icons — clean stroke style matching Home / Non-Conformance
...
// T5 Forms categories — stored as subfolders under tier-5-forms/
...
interface DocumentFile {
  ...
  category?: string // For T5 Forms — tracks which subfolder it came from
}
...
export default function DocumentsPage() {
  ...
  // Auto-expand the Forms submenu when T5 is active
  useEffect(() => {
  ...
  const checkUserAndLoadDocuments = async () => {
    ...
    // Check if user is an Editor (only matters if they're not admin)
    if (!admin) {
                ...
                return (
                  <div key={tier.tierId} className="mb-0.5">
                    <button
                      ...
                      className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-left transition-all ${active ? 'bg-emerald-50 text-emerald-950 shadow-sm' : 'hover:bg-emerald-800/50 text-emerald-100'}`}
                    >
